...
namespace GetThePageRank.DLL
{
    ...
    public static class GooglePR
    {
        ...
        public static int MyPageRank(string MyUrl, string ProxyURL = "")
        {
            if (Uri.IsWellFormedUriString(MyUrl, UriKind.RelativeOrAbsolute))
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(ProxyURL))
                        HttpRequest.Proxy = new WebProxy(ProxyURL);
        ...
        public static int MyPageRankFromMail(string Mail)
        {
            string[] parts = Mail.Split(char.Parse("@")).ToArray();
            if (parts.Count() == 2)
            {
                return MyPageRank(string.Format("http://www.{0}", parts[1]));
            }
            else
            {
                return -1;
